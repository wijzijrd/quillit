...
export function createEntryMention(getEntries: () => Entry[]) {
  return Mention.configure({
    ...
    suggestion: {
      ...
      render() {
        ...
        return {
          onStart(props) { selectedIndex = 0; currentProps = props; build(); paint(props) },
          onUpdate(props) { selectedIndex = 0; currentProps = props; paint(props) },
          onKeyDown({ event }) {
            ...
            if (event.key === 'Enter') { const item = currentProps.items[selectedIndex]; currentProps.command({ id: item.id, label: item.title }); return true }
            return false
          },
