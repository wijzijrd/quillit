...
| `push` | `[<web_project>]`; `--apply`; `--on-conflict fail\|skip\|overwrite\|suffix`; `--create-facets`; `--entry <path>`; `--output <file>`; `--delta` | P1 | Import this project into a web app project (issue #46). Dry-run by default; `--output` writes the tarball to disk instead. `--delta`: only pack/upload entries new or changed since the target project's current state (content-hash comparison against the target); mutually exclusive with `--entry` and `--output`; implies `--on-conflict overwrite` unless `--on-conflict` is passed explicitly. | `quillit push curse-of-strahd --apply` |
